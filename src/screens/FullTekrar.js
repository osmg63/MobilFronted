...
const FullTekrar = ({ navigation, route }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://192.168.137.1:8080/api/words/${userId}`);
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        getItemLayout={(data, index) => ({
          length: 420, // Öğenin uzunluğu + margin/border/padding
          offset: 420 * index, // Önceki öğelerin toplam uzunluğu
          index
        })}
      />
